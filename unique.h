...
public:
		...
		const
	{
        ...
        if(hasSecbest) {
            if(s.paired()) {
                assert_geq(s.bestCScore(), s.bestUnchosenCScore());
                equalSecbest = s.bestCScore() == s.bestUnchosenCScore();
            } else {
                assert_geq(s.bestScore(mate1), s.bestUnchosenScore(mate1));
                equalSecbest = s.bestScore(mate1) == s.bestUnchosenScore(mate1);
            }
        }
